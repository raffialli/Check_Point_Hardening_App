...
The desktop results surface uses a `250px minmax(0, 1fr)` grid: navigation spans both rows on the left, and checks sit above evidence on the right. The check list scrolls within a 230px maximum height; evidence uses the entire remaining width and natural document scrolling. Infrastructure/Categories tabs live at the top of the navigation rail. Neutral 20px outline device icons distinguish management, gateways and clusters; member labels sit below their object name and members follow their cluster. Findings use aligned Check, Status and Severity columns with category bands. The workspace uses 24px horizontal padding and a 10px section gap. One summary counts every original check, including Informational and unfamiliar statuses; Reviewed remains distinct from Passed. Scan timestamps appear separately without repeated counts or empty previous-scan text.

At 1199px and below, navigation becomes 210px and workspace horizontal padding becomes 12px. At 900px it becomes 190px and filters wrap into two rows. On screens up to 760px wide or 650px high, selecting a check enters evidence with keyboard focus; Back to checks restores the selected row without rebuilding or resetting list scroll. At 560px and below, the layout stacks and navigation hides only while viewing evidence. There is no Expand mode. Evidence tables retain unbroken headers, normal word wrapping, and keyboard-focusable horizontal scrolling when their content genuinely exceeds the available width. Wide tables include a visible scrolling hint.
